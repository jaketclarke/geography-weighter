...
if run:
    ...
    df.to_csv("output/2016Census_G03_VIC_SED_2022_unpivot.csv", index=False, na_rep='Null')

    # sort
    out = df.sort_values(by=["census_variable", "value"])
    out.to_csv("output/2016Census_G03_VIC_SED_2022_unpivot_sorted.csv", index=False, na_rep='Null')
    ...
    out.to_csv("output/2016Census_G03_VIC_SED_2022_unpivot_sorted_ranked.csv", index=False, na_rep='Null')

    # pivot
    test = out.pivot(index="census_variable", columns="district", values="value")
    test.to_csv("output/2016Census_G03_VIC_SED_2022_pivot.csv", na_rep='Null')
